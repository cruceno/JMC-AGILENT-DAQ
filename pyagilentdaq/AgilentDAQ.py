...
print ('termine de importar lo standar')
from pyagilentdaq.gui.mainwindow import Ui_jmc_agilent_daq

print ('termine de importar')
# Manejo numerico de datos
import numpy as np
# Funcion para pasar datos como si fueran un archivo de texto
from io import StringIO

from pyagilentdaq.instruments import MultimetrosAgilent
from pyagilentdaq.workers.loopdaq import daq_worker
from pyagilentdaq.ploter.QtMatplotLibPlot import canvas
from pyagilentdaq.ploter.QtMatplotLibPlot import NavigationToolbar
...
class JMCAGILENTDAQ (QtGui.QMainWindow, Ui_jmc_agilent_daq):
    ...
    def init_plot_figure(self):
        ...
        self.mainbar = NavigationToolbar(self.maincanvas,
                                         self.gb_daq_mainplot)
    ...
    def connect_thread(self):
        ...
        self.connect(self.daq,
                     QtCore.SIGNAL("readsignal(PyQt_PyObject)"),
                     self.show_data)
    ...
    def load_available_devices(self):
        ...
        if len(resources_list) == 0:
            ...
            self.statusbar.showMessage('ERROR: Please check the conection\
    ...
    def show_data(self, data):
        if self.daq.mode == 'monitor':
            ...
            # Ploteando en los canvas ya definidos con las funciones heredadas
            # de la clase canvas
            ...
            # self.maincanvas.axes.set_xlim(self.sb_xmin.value(),
            #                                self.sb_xmax.value())
            # self.maincanvas.axes.set_ylim(self.sb_ymin.value(),
            #                               self.sb_ymax.value())
    ...
    def on_pb_start_pressed(self):
        ...
        if self.pb_start.text() == 'Start':
            ...
            if self.le_output_file.text() == '':
                self.daq.adquire(multimeter,
                                 None,
                                 int(self.sb_data_per_second.value()),
                                 'monitor'
                                 )

            else:
                self.daq.adquire(multimeter,
                                 str(self.le_output_file.text()),
                                 int(self.sb_data_per_second.value()),
                                 'datalog'
                                 )
    ...
    def on_tlb_output_file_pressed(self):
        ...
        pathtosave = fd.getSaveFileName(self,
                                        caption='Seleccionar archivo',
                                        directory=os.path.expanduser('~')
                                        )
        ofile = os.path.splitext(str(pathtosave))[0]
